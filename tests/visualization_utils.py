...
def _cleanup_old_artifact_folders(max_folders: int = 5) -> None:
    ...
    if len(timestamped_dirs) >= max_folders:
        folders_to_remove = timestamped_dirs[
            : -max_folders + 1
...
def plot_threshold_to_afhp_mapping(
    samples: List[SamplePoint], test_name: str = "test"
) -> Optional[Path]:
    """
    ...
    filename = "threshold_to_afhp.png"
...
def plot_afhp_to_return_mapping(
    ...
    if phase1_afhp:
        plt.scatter(
            ...
            s=50,
            c="orange",
            label=f"Phase 1 ({len(phase1_afhp)} samples)",
        )
    ...
    if phase2_afhp:
        plt.scatter(
            ...
            s=50,
            c="red",
            label=f"Phase 2 ({len(phase2_afhp)} samples)",
        )
...
def save_test_artifacts(
    ...
    threshold_plot = plot_threshold_to_afhp_mapping(plot_samples, test_name)

    # Generate AFHP-to-return plot
    return_plot = plot_afhp_to_return_mapping(plot_samples, sampler, test_name)
        ...
        for artifact_type, filepath in artifacts.items():
            if artifact_type != "directory":  # Skip the directory entry
                if filepath:
                    ...
                    print(
                        f"  - {artifact_type}: Not generated (matplotlib unavailable)"
                    )
